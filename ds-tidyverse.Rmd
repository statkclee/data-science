...
layout: page
title: 데이터 사이언스
subtitle: "`tidyverse` 데이터 사이언스 기본체계"
author:
    name: xwMOOC
date: "`r Sys.Date()`"
output:
...
`tidyverse`와 `messyverse` 차이를 이해하고, 사람과 컴퓨터를 작업 최적화를 위한 접점을 찾아본다. 데이터 사이언스 문제에 대한 R 진영 `tidyverse` 해결책을 이해한다.

# 데이터 과학 문제정의 {#tidyverse-messyverse}
...
<img src="fig/data-science-rationale.png" alt="전통적 문제정의" width="77%" />
...
# 문제해결 개념 아키텍쳐 {#tidyverse-problem-architecture}
...
<img src="fig/data-science-countermeasure.png" alt="R진영 해결책" width="77%" />

##`dplyr` 동사 명령어 {#tidyvrse-dplyr}
...
전통적 R 코드와 파이프 연산자를 사용한 R 코드 비교하면 다음과 같다.
...
```{r traditional-r-code, eval=FALSE}
hourly_delay <- filter(   
    summarise(   
        group_by(   
             filter(   
...
# `tidyverse` 선언 [^tidyverse-menifesto] {#tidyverse-manifesto}
...
# 프로그래밍과 데이터 사이언스 작업흐름 [^jenny-plotcon-2016] {#tidyverse-workflow}
