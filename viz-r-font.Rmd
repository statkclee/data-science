...
>
> * R 그래프에 한글 글꼴을 적용한다.
> * `extrafont`와 `showtext` 팩키지를 활용하여 한글 폰트를 사용한다.
...
``` {r showtext-showtext-install, eval=FALSE}
# 1. 기본(Base) 그래픽 --------------------------------------------------------------------------
...
``` {r showtext-showtext-ggplot, eval=FALSE}
# 2. ggplot 그래픽 --------------------------------------------------------------------------
...
``` {r showtext-showtext-korean, eval=FALSE}
# ３. 한글 그래픽 --------------------------------------------------------------------------
## 나눔펜　스크립트
font.add("Nanum Pen Script", "Nanum Pen Script.ttf")
...
p <- ggplot(NULL, aes(x = 1, y = 1)) + ylim(0.8, 1.2) +
  ...
  annotate("text", 1, 1.1, family = "Nanum Pen Script", size = 15, color="red",
           label = "한글 사랑") +
  annotate("text", 1, 0.9, label = 'korean for "Hello, world!"',
           family = "Nanum Pen Script", size = 12)
