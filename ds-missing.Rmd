...
apply(data, 2, check_missing_value)
apply(data, 1, check_missing_value)
```
...
혹은 `VIM` 팩키지를 활용하여 결측값을 시각화하는 것도 좋다.

``` {r boston-housing-vim, warnings=FALSE}
library(VIM)
aggr_plot <- aggr(BostonHousing, col=c('ptratio','rad'), numbers=TRUE, 
	sortVars=TRUE, labels=names(data), cex.axis=.7, gap=3, ylab=c("Histogram of missing data", "Pattern"))

marginplot(BostonHousing[, c('ptratio','med'))
```

`marginplot(BostonHousing[, c('ptratio','med'))` 그래프를 통해 변수 2개만 가능하지만, 결측값이 포함된 경우와 결측값이 없는 경우를 비교해 볼 수도 있다.
