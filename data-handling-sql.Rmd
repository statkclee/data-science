...
<img src="fig/dplyr-dbi-sqlite.png" alt="dplyr, dbi, sqlite3 변환" width="100%" />
...
# 데이터모델 - `datamodelr` [^datamodelr] {#sqlite3-survey-model}

[^datamodelr]: [Data model diagrams in R](https://github.com/bergant/datamodelr)

`survey.db` 데이터베이스 정보를 바탕으로 데이터프레임을 추출한다. 
이를 `datamodelr`에 넣어 데이터베이스 모델을 시각화한다.
[`datamodelr`](https://github.com/bergant/datamodelr) 팩키지에 나와 있는 `nycflights13` 사례를 참조하여 유사하게 제작한다.

먼저, 데이터프레임만 시각화한다.

```{r survey-database-model}
library(datamodelr)

survey <- tbl(con, "Survey") %>% collect()
person <- tbl(con, "Person") %>% collect()
site <- tbl(con, "Site") %>% collect()
visited <- tbl(con, "Visited") %>% collect()
