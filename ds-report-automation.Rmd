...
<img src="fig/ds-old-report-workflow.png" alt="기존 보고서 작성 작업흐름" width="77%" />
...
<img src="fig/ds-new-report-workflow.png" alt="보고서 작성 자동화 작업흐름" width="77%" />
...
```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE}
stargazer(df[,-1], type="html", notes.align = "l")
```
...
step(df_null, scope=list(lower=df_null, upper=df_full), direction="forward")
# step(df_null, scope=list(lower=df_null, upper=df_full), direction="both")
```
...
```{r, results='asis', echo=FALSE, warning=FALSE, message=FALSE}
# 최상부분집합선택법, 
# 참조: http://www.stat.umn.edu/geyer/5102/examp/select.html

subset_mod <- regsubsets(mpg ~., data=df[,-1], 
                         nvmax = NULL,
...
# 최종회귀모형 상세
stargazer(mtcars_best, type="html", notes.align = "l")
```
