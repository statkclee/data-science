...
# R로 표 작성 [^DescTools-table] {#categorical-desctools}
...
# 범주형 데이터 자료구조 {#categorical-data-structure}

표를 작성하는데 있어 가장 먼저 확인해야 되는 사항은 범주형 자료구조를 확인하는데 있다.

1. **관측점 하나에 빈도수 하나인 경우(Case-by-Case)**: `data.frame` 혹은 행렬에 원자료에 각행에 사례 한가지만 기록되어 저장되어 있는 경우.

`Untable()` 함수는 데이터프레임을 입력받아 관측점 하나에 빈도수 하나인 데이터프레임으로 변환시킨다.

``` {r table-ubc-admission}
library(tidyverse)
library(DescTools)
...
```{r ubcadmin}
DescTools::Untable(UCBAdmissions) %>% 
  head()
```
...
# 표 데이터 자료형 변환 {#table-to-dataframe}

표데이터를 다음과 같이 `Case-by-Case`, `빈도수 가중치`, `표` 세가지 자료형이 존재한다.

1. **관측점 하나에 빈도수 하나인 경우(Case-by-Case)**: `d_col <- Untable(HairEyeColor)`
1. **빈도수(Frequency)**: `d.weight <- as.data.frame(HairEyeColor)`
1. **표(Table)**: `tab <- HairEyeColor`

- `사례별(Case-by-Case)` &rarr; `빈도수(Frequency)` :  `as.data.frame(table(d_col))`
...
- `사례별(Case-by-Case)` &rarr; `표(Table)` :  `table(d_col)`
...
![사례별, 빈도수, 표 변환](fig/case-freq-table.png){#id .class width="100%"}

# 표 생성 {#categorical-create-table}
...
1. `TextToTable()` 함수 활용 : `DescTools` 팩키지에서 지원되는 함수를 사용한다.
...
## 숫자형 변수에서 범주형 자료 생성 {#desctools-categorical}
...
# 표 만들기(Tabulate) {#create-tablulate}
...
## 단변량 범주형 변수 {#create-table-univariate}

``` {r HairEyeColor-univate}
d_col <- Untable(HairEyeColor)
d_col %>% 
  dplyr::filter(Sex=="Male", Hair=="Blond", Eye=="Brown")
```

요인형 자료를 갖고 표를 생성시키는 방법은 `table` 함수로 빈도수, `prop.table` 함수로 백분율을 표로 표현한다.

``` {r HairEyeColor-pro-table}
table(d_col$Hair)
prop.table(table(d_col$Hair))
```

`Freq` 함수를 사용해서 단변량 범주형 변수의 경우 빈도수와 백분율을 간단히 표로 표현한다.

``` {r HairEyeColor-freq}
Freq(d_col$Hair, ord="desc")
```

## 이변량 범주형 변수 {#create-bivariate}
...
``` {r create-tabl-old}
with(d_col, table(Hair, Sex))
```

혹은 `plyr`, `dplyr` 팩키지의 `count()` 함수를 사용하면 `table`, `as.data.frame` 단계를 거치지 않고 일관된 방식으로 
표생성에 필요한 데이터를 만들어 낼 수 있다. 이를 `xtabs` 함수와 결합하여 표 

``` {r create-table-xtabs}
dplyr_tbl <- d_col %>% count(Hair, Sex)
xtabs(n~Hair+Sex, dplyr_tbl)
```

``` {r create-table-three-ways}
with(d_col, prop.table(table(Hair, Eye), 1)) # 행기준
with(d_col, prop.table(table(Hair, Eye), 2)) # 열기준  
with(d_col, prop.table(table(Hair, Eye), margin=NULL)) # 전체
```
...
``` {r create-table-perctable}
PercTable(Hair ~ Eye, data=d_col, rfrq="111", margins=c(1,2))
```
...
## `formattable` {#gt-formatable-package}

[`formattable`: Formattable Data Structures](https://github.com/renkun-ken/formattable) 팩키지를 사용하여 표를 꾸밀 수 있다.

```{r formattable-example}
library(formattable)
...
## `gt` [^gt-github] [^gt-case-study] {#table-gt}

[^gt-github]: [`gt`: Easily generate information-rich, publication-quality tables from R](https://github.com/rstudio/gt)

[^gt-case-study]: [`gt`: Case Study - gtcars](https://gt.rstudio.com/articles/case-study-gtcars.html)

[`gt`](https://gt.rstudio.com/) 팩키지는 표(table)을 각 부분으로 나눠서 `티블(tibble)` 혹은 `데이터프레임(data frame)` 객체를 `gt` 객체로 변환시킨 후에 `gt` 표(테이블)로 만들어서 각종 문서에 사용할 수 있다.

먼저 `gt` 테이블 구성요소를 다음과 같이 구성할 수 있고, 이를 `gt` 작업흐름에 맞춰 작업한다.
즉 표는 다음을 구성요소로 갖는다.
...
![`gt` 테이블 구성요소](fig/gt_parts_of_a_table.svg)

일반적인 `gt` 테이블 작업흐름은 앞서 언급한 것처럼 `tibble` 혹은 `data.frame` &rarr; `gt` 객체 &rarr; `gt` 테이블이 된다.

![`gt 테이블 작업흐름`](fig/gt_workflow_diagram.svg)

`UCBAdmissions` 데이터셋을 가지고 `gt` 테이블을 제작해 보자.
