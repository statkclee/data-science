...
# 요인 자료형 [^stat545-factors] {#stat545-factor}
 ...
 *절대로 잊지 말아야 되는 사실은* R에서는 정수 1,2,3... 으로 저장된다는 점이다.
 ...
 요인형의 야누스적인 특성으로 말미암아 여러가지 불미스런 사건을 많이 일으키기도 하지만 **필요악**으로,  단점보다 훨씬 장점이 더 많다. 특히 모형개발이나 그래프를 생성할 때, 요인은 항상 따라다니고 잘 다뤄야 하는 필수적인 자료형이  요인형이다.

가장 최악의 요인형은 탐지되지 않는 요인이다. 문자형이라고 생각했던 변수가 실제로 컴퓨터 R은 요인형(정수 숫자형)으로 이해하고 있다는 점이다. 그럼 **탐지되지 않는 요인(stealth factor)**은 어디서 나온 것일까? Base R은 문자형 변수를 요인형으로 자동 변환하는데 데이터를 불러 읽어 들일 때 
...
# `forcats` 팩키지 불러오기 {#stat545-forcats}

과거 `tidyverse`의 핵심 팩키지가 아니라서 `install.packages("tidyverse")` 명령어로 설치하고 `library(forcats)` 명령어로 적재시켜야 한다. 이제는 `tidyverse`의 일원이 되어서 `library(tidyverse)`로 손쉽게 작업할 수 있게 되었다. `forcats` 팩키지에서 범주형 변수를 다루기 위해서 `fct_` 함수명을 접두어로 두고 활용한다.

``` {r import-forcats}
library(tidyverse) # 혹은, library(forcats)
library(gapminder)
```


# `forcats` 요인 자료 다루기 {#stat545-forcats}

## 요인(factor) 범주 살펴보기 {#stat545-forcats-glance}
...
## 사용되지 않는 수준(level) 제거 {#forcats-droplevels}
...
## 수준(level) 순서 변경 {#forcats-relevel}
...
## 수준 명칭변경 다시 정의 {#forcats-recode-label}
...
## 범주 요인 결합 {#forcats-combine}

요인변수를 결합하는 경우 단순히 `c()`를 사용하게 되면 결합이 뜻한대로 되지 않는 경우가 있다. 이를 방지하기 위해서 `fct_c()` 함수를 사용하면 안전하게 결합시킬 수 있다.
...
마찬가지로 데이터프레임에 포함된 범주형 변수를 다룰 때 `bind_rows()` 함수와 `rbind()`는 다르게 동작 한다. 즉, `bind_rows()` 함수는 수준이 다르기 때문에 요인을 문자열로 변환시켜 후속 작업을 준비한다.

```{r forcats-bind_rows}
bind_rows_df <- bind_rows(df1, df2)
bind_rows_df
```

반면에, `rbind()` 함수는 범주형 요인으로 계속 자료형을 유지한다.
